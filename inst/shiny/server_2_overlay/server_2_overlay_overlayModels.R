...
overlay_all <- eventReactive(input$overlay_create_overlaid_models, {
  ...
  withProgress(message = 'Model overlay step:', value = 0.1, {
    ...
    if (identical(overlay_crs(), crs.ll)) {
      ...
      models.to.overlay <- lapply(vals$models.orig[-base.idx], function(sf.obj) {
        # Project sf.obj if necessary
        if (!identical(overlay_crs(), crs(sf.obj))) {
          sf.obj <- spTransform(sf.obj, overlay_crs())
        }

        # Make sf.obj valid if it is not
        if (!all(st_is_valid(sf.obj))) {
          sf.obj <- valid.poly(sf.obj, "Overlaid model predictions")
        }
    ...
    browser()
    names(base.sf) <- c("Pred.overlaid", "Error.overlaid",
    ...
    base.specs[3:4] <- NA
    if (identical(crs(base.sfc), crs.ll)) {
      base.sfc.ll <- base.sfc
    } else {
      base.sfc.ll <- st_transform(base.sfc, crs.ll)
    }
    base.specs[5] <- paste(as.character(round(extent(base.sfc.ll), 0)),
                           collapse = ", ")
    ...
    models.overlaid.all <- mapply(function(model, prog.num) {
      ...
      overlay.func(base.sfc, model, overlap.perc)
