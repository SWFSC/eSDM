...
polyValidCheck <- function(poly.invalid, dens.col = NA, poly.info = NA) {
  ...
  if (!all(st_is_valid(poly.maybe))) {
    ...
    alert2 <- "The eSDM made the polygon valid using the st_make_valid() function from the lwgeom package." # TODO make this better
    alert3 <- paste(
      "The difference between the area of the valid polygon and the area of the original polygon is",
      round(area.dif / 1e+06, 2), "square km, which is",
      round((area.dif / area1) * 100, 2), "percent different than the area of the original polygon."
    )
    if (!is.na(dens.col)) {
      alert4 <- paste(
        "The difference between the predicted abundance of the valid sdm and the predicted abundance of the original sdm is",
        round(abund.dif, 0), "animals, which is",
        round((abund.dif / abund1) * 100, 2), "percent different than the predicted abundance of the original sdm."
      )
    } else {
      alert4 <- NULL
    }
