...
overlay_land_gis_shp <- reactive({
  ...
  withProgress(message = 'Loading land polygon', value = 0.3, {
    bound.sf <- read.shp.shiny(input$overlay_land_gis_shp_files)
    ...
    vals$overlay.land <- overlay.gis.crs(st_geometry(bound.sf))
    incProgress(0.2)
  })
...
overlay_land_gis_gdb <- eventReactive(input$overlay_land_gis_gdb_load, { 
  ...
  withProgress(message = 'Loading land polygon', value = 0.3, {
    bound.sf <- try(st_read(input$overlay_land_gis_gdb_path, 
