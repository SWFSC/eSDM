...
This document contains code for plotting predictions; by default some of the plotting code is not run because it can take several minutes (these code chunks contain the comment "code not run").

To use data from this example analysis, please first contact the author and then see the README.txt file for proper citation information (located at `system.file("extdata/README.txt", package = "eSDM"`).


## Import SDM predictions

The first step of the example analysis is to import and process the Model_B, Model_H, and Model_R predictions, along with their standard error (SE) values. Use `pts2poly_centroids` to create `sf` objects from polygon centroids from .csv files, `raster::raster` to import raster files, and `sf::st_read` to import GIS files. The dimensions of the Model_B and Model_H predictions are 0.09 x 0.09 degrees and 0.25 x 0.25 degrees, respectively; the second argument of `pts2poly_centroids` is half the length of one side of the polygon. GIS files already have a defined geometry that is read by `st_read`. When importing and processing predictions, you must ensure the following:
...
After importing and processing the SDM predictions, we can visualize them. These plots make up Fig. 3 in Woodman _et al._ (in review). This vignette uses the custom `plot_sf_3panel` function (in 'vignette_helper.R') for plotting. `plot_sf_3panel` and `tmap_sdm` (used later in this vignette) were not included as functions in the `eSDM` package because they are fairly specific to the example analysis region and SDMs. However, they can provide a framework for plotting SDMs using the plotting functionality of the `sf` and `tmap` packages, and you should be able to adapt these functions to your specific plotting needs.
...
```{r, eval=FALSE}
# Convert raster to sf object; code not run
logo <- raster::raster(system.file("external/rlogo.grd", package="raster"))
logo.sf <- sf::st_as_sf(as(logo, "SpatialPolygonsDataFrame"))
```
...
Next, we must convert any associated uncertainty values from to variance; uncertainty values must be overlaid as variance values to remain statistically valid. 
...
Now we can calculate the AUC and TSS metrics for the original and overlaid predictions. The displayed table is from Table 3 of Woodman _et al._ (in review). Note that `evaluation_metrics` requires that validation data have the same crs as the predictions being evaluated.
...
We can see that the prediction values are now much more comparable, meaning a subset of the predictions will not contribute disproportionately to an ensemble.

We also must calculate the ensemble weights, which must be manually created. For the example analysis, we use several weighting methods: equal weights (i.e., unweighted), AUC-based weights, TSS-based weights, and weights calculated as the inverse of the prediction variance. Each set of weights must sum to 1, or each row must sum to 1 in the case of the weights calculated as the inverse of the prediction variance.
...
Now we can calculate AUC and TSS scores for the ensembles and compare them to those of original and ensemble predictions. Again, the evaluation code is not run and the displayed table is Table 3 from Woodman _et al._ (in review).
