...
model.b.sf <- read.csv(system.file("extdata/Predictions_Beckeretal2016.csv", package = "eSDM")) %>% 
...
model.h.sf <- read.csv(system.file("extdata/Predictions_Hazenetal2017.csv", package = "eSDM")) %>%
...
model.r.sf <- st_read(system.file("extdata/Shapefiles/Predictions_Redfernetal2017.shp", package = "eSDM")) %>%
  st_make_valid() %>% #
  st_set_agr("constant")
...
We could also have estimated the wintin-model ensemble uncertainty by using the `x.var.idx` argument, as demonstrated in the following code (not run).
